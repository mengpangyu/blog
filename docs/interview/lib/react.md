...
在 React 函数组件中，每次渲染都会创建一份新的闭包环境。当 `useEffect` 的依赖项没有变化时，effect 函数不会被替换，内部引用的 state/props 仍然是旧的闭包环境中的值，导致拿不到最新状态，这就是**闭包陷阱**。
...
}, []) // 依赖为空,effect 永远持有初始闭包
```

上例中 `count` 永远是 `0`，因为 `[]` 依赖使得 effect 不会重新创建，闭包中捕获的 `count` 始终是初始值。

### 解决方案

**1. 函数式更新**

不从闭包中读取旧值，而是通过回调从 Fiber 节点获取最新值：
...
将 effect 中使用的变量加入依赖数组，让 effect 在变量变化时重新创建：
...
`useRef` 的 `.current` 不受闭包限制，始终可以拿到最新值：
...
声明一个事件函数，每次调用时自动获取最新的 state/props，且不需要作为 effect 的依赖项：

```tsx
const handle = useEffectEvent(() => {
  console.log(count) // 始终拿到最新 count
})
